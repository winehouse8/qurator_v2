...
2. 두 번째 카드부터 두 가지 레이아웃 중 하나를 택해 일관되게 사용한다.
   - with_sub_title_body : sub_title, body 키 포함
   - only_sub_title   : sub_title 키만 포함
   - only_body   : body 키만 포함
...
3. 만약 검색 결과상 흥미로운 내용은 별로 없고 사진이 메인이 되는 카드뉴스라고 생각되면, only_sub_title 모드로 두번쨰 페이지부터는 sub_title만 붙히고 사진에 집중한다.
4. 여러 장소/가게/패션아이템 등을 소개하는 카드뉴스라면, 각 페이지의 sub_title에는 특정 장소/가게/패션아이템 을 특정할 수 있도록 적어야 한다.
...
5. 여러 장소/가게를 소개하는 경우, body에는 해당 장소 또는 가게에 대한 간략한 장점,특징, 주요정보(주요메뉴,기간/시간 등)를 쓰는것이 좋다.(그런 정보를 가지고 있는 경우에 한함)
6. with_sub_title_body/only_body 를 선택하는 기준: 소개할 내용이 길지 않다면 2장(title 만 한장, body만 있는 한장)으로 충분한 경우가 있다.(only_body 모드). 보통 특정 사건(뉴스)를 소개하는 경우,  2장의 only_body 모드를 쓰는 것을 기본으로 한다. 하지만 소개할 흥미로운 내용이 많은 경우 뉴스라 할지라도 여러장을 허용한다.(with_sub_title_body 모드) 즉 소개할 흥미로운 내용의 양이 많은지가 모드 선택의 주요 판단 기준이 된다.
7. 카드뉴스는 두괄식이 좋다. 그래서 sub_title에는 title을 보고 사람들이 가장 궁금해할 만한것을 직접적으로 적어주는것이 좋다. 예를들어 title이 애니추천 5선이면 애니 이름을 sub_title에 적는식이다.
